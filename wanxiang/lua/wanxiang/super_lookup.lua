  ...
  if env.has_comment then
    ...
    if comment_text ~= "" then
      ...
      if not comment_cache[cache_key] then
        comment_cache[cache_key] = parse_comment_codes(comment_text, env.comment_split_ptrn, cand_len, env.enable_tone) or
        false
...
local function attempt_phrase_correction(cand, cand_len, env, syllables, fuma_chunks, syl_offset)
  ...
  elseif #fuma_chunks == 1 then
    new_text, count, next_end = try_match_two_char_phrase(current_text, search_end_idx, env, syllables, fuma_chunks[1],
      syl_offset)
    if new_text then
      current_text = new_text
      match_count = count
  ...
  if match_count == 0 then
    current_text, match_count = try_match_single_chars(current_text, search_end_idx, env, syllables, fuma_chunks,
      syl_offset, match_count)
  end
  ...
  for cand in input:iter() do
    ...
    if is_first_cand then
      ...
      if apply_tone_filter and clean_fuma == "" then
        ...
        local tone_cand = attempt_pure_tone_translation(cand, env, syllables, tone_filter_seq, current_syl_count,
          syl_offset)
        if tone_cand then
          yield(tone_cand)
          goto skip
