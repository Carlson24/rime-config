...
local function get_db(env)
  local db_name = "lua/userdata/predict"
      ...
      for i, c in ipairs(prefix_cands) do
        if i <= CONFIG.MAX_MEMORY_BRANCHES then
          if not seen[c.word] then
            insert(cands, c); seen[c.word] = true
          end
        else
...
function P.fini(env)
  if env.commit_connection then
    env.commit_connection:disconnect(); env.commit_connection = nil
  end
  if env.update_connection then
    env.update_connection:disconnect(); env.update_connection = nil
  end
  if env.delete_connection then
    env.delete_connection:disconnect(); env.delete_connection = nil
  end
end
...
function F.func(input, env)
  ...
  if CONFIG.PREDICT_STYLE ~= "reorder" then
    for cand in input:iter() do yield(cand) end
    return
  end
  ...
  if (not do_reorder) or current_input == "" then
    for cand in input:iter() do yield(cand) end
    return
  end
  ...
  for cand in input:iter() do
    ...
    if ct == "raw" or ct == "english" or is_alpha_fast(text) or length_mismatch_stop or count > max_scan then
      ...
      flush_yield(shared_boosted, b_cnt, shared_normal, n_cnt)
      yield(cand)
