...
local tones_map = wanxiang.tone_mark_digit

local get_utf8_len = utf8.len

local tone_key_map = wanxiang.tone_key_map

local function get_tone_from_pinyin(pinyin)
  if not pinyin or #pinyin == 0 then
    return nil
  end
  local digit = pinyin:match("([1-5])$")
  if digit then
    return tone_key_map[digit]
  end
  pinyin = wanxiang.normalize_v(pinyin)
...
-- 解析输入的辅码，仅将 6,7,8,9,0 视为声调，其余为常规辅码
local function parse_aux_rules(aux)
  ...
  for i = 1, #aux do
    local char = aux:sub(i, i)
    if char == "6" or char == "7" or char == "8" or char == "9" or char == "0" then
      table.insert(tone_filter_seq, char)
    else
      clean_aux = clean_aux .. char
    end
  end
  ...
  return clean_aux, tone_filter_seq, aux_chunks
end

local function parse_comment_part(part, enable_tone, env)
  local pinyin_part = part
  local codes_part = ""

  -- 统一格式：pinyin;aux（单分号），取分号前为拼音、分号后为辅助码
...
local function parse_comment_codes(comment, pattern, target_len, enable_tone, env)
  if not comment or comment == "" then
    return nil
  end

  if target_len == 1 then
    return { parse_comment_part(comment, enable_tone, env) }
  end
  ...
  for part in comment:gmatch(pattern) do
    count = count + 1
    result[count] = parse_comment_part(part, enable_tone, env)
  end
...
local function ensure_lookup_resources(env)
  ...
  if main_rules and #main_rules > 0 then
    env.main_projection = Projection()
    env.main_projection:load(main_rules)
  end
  if xlit_rules and #xlit_rules > 0 then
...
-- 5. 候选项数据构建核心
local function ensure_db_cache_entry(env, code_point, need_xlit)
  local db_cache = env._db_cache
  local entry = db_cache[code_point]
  ...
  if env.has_comment then
    ...
    if comment_text ~= "" then
      ...
      if parsed_comment == nil then
        parsed_comment = parse_comment_codes(comment_text, env.comment_split_ptrn, cand_len, env.enable_tone, env)
            or false
...
-- 综合匹配判断引擎 (引导模式使用)
local function check_explicit_match(raw_data, cand_len, clean_aux, tone_filter_seq, apply_tone_filter, env)
    ...
    if codes_seq then
      ...
      if tone_match_pass then
        if source_type == "aux" or source_type == "db" then
          if cand_len == 1 then
            if group_match(codes_seq[1], clean_aux) then
              return true
            end
          else
            local memo = {}
            if match_fuzzy_recursive(codes_seq, 1, clean_aux, 1, memo, source_type == "db") then
              return true
            end
          end
        end
...
-- 7. 动态引擎逻辑判定提取 (动态模式使用)
local function check_direct_match(raw_data, cand_len, clean_aux, data_sources)
  for _, source_type in ipairs(data_sources) do
    ...
    if codes_seq then
      ...
      elseif cand_len == 2 then
        ...
        if fl == 1 then
          if
              match_direct_word(codes_seq, 1, clean_aux, is_db)
              or match_direct_word(codes_seq, 2, clean_aux, is_db)
          then
            return true
          end
        elseif fl == 2 then
          local case1 = match_direct_word(codes_seq, 1, clean_aux, is_db)
          local case2 = match_direct_word(codes_seq, 2, clean_aux, is_db)
          local case3 = false
          if
              match_direct_word(codes_seq, 1, clean_aux:sub(1, 1), is_db)
              and match_direct_word(codes_seq, 2, clean_aux:sub(2, 2), is_db)
          then
            case3 = true
          end
...
local function create_direct_candidate(cand, ctx_input, pure_code, aux)
  ...
  if orig_preedit and orig_preedit ~= "" then
    ext_cand.preedit = orig_preedit:gsub("%s+$", "") .. " " .. aux
  else
    ext_cand.preedit = pure_code .. " " .. aux
  end
...
local function handle_explicit_mode(input, env, ctx_input, pure_code, explicitly_aux, s_end)
  ...
  local clean_aux, tone_filter_seq, aux_chunks = parse_aux_rules(explicitly_aux)
  ...
  for cand in input:iter() do
    ...
    if is_first_cand then
      ...
      if apply_tone_filter and clean_aux == "" then
    ...
    if
        raw_data and check_explicit_match(raw_data, cand_len, clean_aux, tone_filter_seq, apply_tone_filter, env)
    then
      has_any_match = true
  ...
  if not has_any_match and apply_tone_filter and #clean_aux > 0 and env.has_db and env.db_table then
    for _, db_obj in ipairs(env.db_table) do
      local res_str = db_obj:lookup(clean_aux)
...
local function handle_direct_mode(input, env, ctx_input)
  ...
  local function build_matches()
    ...
    matched_cands = {}
    matched_text_count = {}
    aux = ctx_input:sub(#base_input + 1):gsub("['%s]", "")
    clean_aux = aux:gsub("[67890]", "")

    if #clean_aux ~= 1 and #clean_aux ~= 2 then
      return
    end

    local raw_cache = env._raw_cache
    for _, saved in ipairs(direct_cache.candidates or {}) do
      local cache_key = saved.text .. "\1" .. (saved.comment or "")
      local raw_data = raw_cache[cache_key]
      if raw_data == nil then
        local cached_cand = restore_direct_candidate(saved)
        raw_data = build_candidate_raw_data(cached_cand, 2, env) or false
        raw_cache[cache_key] = raw_data
      end
      if raw_data and check_direct_match(raw_data, 2, clean_aux, env.data_sources) then
        local cached_cand = restore_direct_candidate(saved)
        local ext_cand = create_direct_candidate(cached_cand, ctx_input, base_input, aux)
  ...
  elseif mode == "lookup" and matched_cands and #matched_cands > 0 and #clean_aux == 2 and not matches_yielded then
    yield_matches()
  end
end
...
function f.init(env)
  ...
  env.notifier = env.engine.context.select_notifier:connect(function(ctx)
    ...
    local code, aux = split_lookup_input(input, env.search_key_str, env.bypass_prefix)
...
function f.func(input, env)
  ...
  local pure_code, explicitly_aux, s_start, s_end = split_lookup_input(ctx_input, env.search_key_str, env.bypass_prefix)
  ...
  if s_start then
    ...
    return handle_explicit_mode(input, env, ctx_input, pure_code, explicitly_aux, s_end)
  else
