...
function M.func(input, seg, env)
  ...
  -- 根据当前激活的上屏方案选择对应的翻译器
  if env.engine.context:get_option("lishi26_quanpin") then
    -- 全拼方案激活
    result = env.translator1:query(input, seg)
  elseif env.engine.context:get_option("lishi26_ddpy") then
    -- 拼音方案激活
    result = env.translator2:query(input, seg)
  elseif env.engine.context:get_option("lishi26_zhuyin") then
    -- 注音方案激活
    result = env.translator3:query(input, seg)
  elseif env.engine.context:get_option("lishi26_guoluo") then
    -- 国罗方案激活
    result = env.translator4:query(input, seg)
  elseif env.engine.context:get_option("lishi26_translator") then
    -- 主翻译器激活
