...
-- 字符串转义
local function escp(ch) return wanxiang.escape_pattern(ch) end
...
local function count_trailing(s, ch)
  local n = 0; for i = #s, 1, -1 do if s:sub(i, i) == ch then n = n + 1 else break end end; return n
end
...
function M.init(env)
  ...
  env.conn_update = context.update_notifier:connect(function(ctx)
    ...
    -- preedit 未变时跳过 O(n) 分段扫描
    if pre ~= env.seg_last_preedit then
      env.seg_last_preedit = pre or ""
