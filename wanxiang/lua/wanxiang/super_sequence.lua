...
local function _file_exists(path)
  ...
  local f = io.open(path, "r"); if f then
    f:close(); return true
  end
  return false
end
...
local function _sync_dir()
  ...
  local base
  if ysync and ysync ~= "" then base = fix(ysync) end
  base = base or _path_join(user_dir, "sync")
  if install_id and install_id ~= "" then
    return _path_join(base, _path_join(install_id, "lua"))
  end
  return base
end
...
local function _sync_ready()
  ...
  if install_id and install_id ~= "" then
    dir = _path_join(dir, _path_join(install_id, "lua"))
  end
...
local function _pending_count()
  local n = 0; for _ in pairs(seq_data.pending_map) do n = n + 1 end; return n
end
...
function seq_data._ensure_export_file()
  ...
  if not seen[export_name] then
    names[#names + 1] = export_name; _write_lines(manifest, names)
  end
  return true
end
...
function seq_data.maybe_export(force)
  if force then
    seq_data.flush_pending(nil); seq_data.last_export_ts = get_timestamp(); return
  end
  if _pending_count() == 0 then return end
            ...
            if line ~= "" and line:sub(1, 2) ~= "\001" .. "/" then
              ...
              if key and value then
                ...
                if item then
                  ...
                  local mp = parse_adjustment_values(value); if mp then for it, a in pairs(mp) do _keep_latest(latest,
                        key, it, a) end end
                end
              end
            end
...
local function rewrite_export_from_latest(latest)
  ...
  local dir = _sync_dir()
  local installation_id = select(1, _read_installation_yaml())
  local device_name = (installation_id and installation_id ~= "") and
  tostring(installation_id):gsub("[%s/\\:%*%?\"<>|]", "_") or "device"
  local export_name = string.format("%s_%s%s", SYNC_FILE_PREFIX, device_name, SYNC_FILE_SUFFIX)
  local export_path = _path_join(dir, export_name)
  local manifest = _manifest_path(dir)

  if not _file_exists(manifest) then
    local mf = io.open(manifest, "w"); if mf then mf:close() end
  end
  do
  ...
  for _, input in ipairs(inputs) do
    ...
    for _, item in ipairs(keys) do
      local a = items[item]
      table.insert(buffer,
        string.format("%s\ti=%s p=%s o=%s t=%s", input, item, a.fixed_position or 0, a.offset or 0, a.updated_at or ""))
    end
  end
  ...
  if old_content ~= new_content then
    ...
    if f_write then
      f_write:write(new_content); f_write:close()
    end
  end
end
...
function P.func(key_event, env)
  ...
  if key_repr == DEFAULT_SEQ_KEY.up then
    curr_state.offset = -1; curr_state.mode = curr_state.ADJUST_MODE.Adjust
  elseif key_repr == DEFAULT_SEQ_KEY.down then
    curr_state.offset = 1; curr_state.mode = curr_state.ADJUST_MODE.Adjust
  elseif key_repr == DEFAULT_SEQ_KEY.reset then
    curr_state.offset = nil; curr_state.mode = curr_state.ADJUST_MODE.Reset
  elseif key_repr == DEFAULT_SEQ_KEY.pin then
...
local function apply_prev_adjustment(cands, prev)
  ...
  for _, info in pairs(prev or {}) do
    if info.raw_position then
      info.from_position = info.raw_position; table.insert(list, info)
    end
  end
...
function F.func(input, env)
  ...
  local curr_adjustment = curr_state.has_adjustment() and
  { fixed_position = 0, offset = 0, updated_at = get_timestamp() } or nil
