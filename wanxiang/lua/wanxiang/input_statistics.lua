...
local function record_stats_mem(env, hanzi_len, code_len)
  ...
  elseif hanzi_len == 2 then
    pending_accum(day_key, "_l2", 1)
  elseif hanzi_len == 3 then
    pending_accum(day_key, "_l3", 1)
  elseif hanzi_len == 4 then
...
local function aggregate_stats(env, days_lookback)
  ...
  if days_lookback == 0 then
    local prefix = "total"
    return {
      len = db_get(db, prefix .. "_len"),
      cnt = db_get(db, prefix .. "_cnt"),
      code = db_get(db, prefix .. "_code"),
      spd = db_get(db, prefix .. "_spd"),
      l1 = db_get(db, prefix .. "_l1"),
      l2 = db_get(db, prefix .. "_l2"),
      l3 = db_get(db, prefix .. "_l3"),
      l4 = db_get(db, prefix .. "_l4"),
      l_gt4 = db_get(db, prefix .. "_l_gt4")
    }
  end
...
local function translator(input, seg, env)
  ...
  if not data then
    ...
    if string.sub(input, 1, trigger_len) == env.triggers.history then
      ...
      local s_y, s_m, s_d, e_y, e_m, e_d = input:match("^" ..
        safe_trigger .. "(%d%d%d%d)(%d%d)(%d%d)t(%d%d%d%d)(%d%d)(%d%d)$")
      if s_y then
        is_matched = true
        title = "区间"
        subtitle = string.format("%s.%s.%s - %s.%s.%s", s_y, s_m, s_d, e_y, e_m, e_d)
        data = aggregate_custom_period(env, tonumber(s_y), tonumber(s_m), tonumber(s_d), tonumber(e_y), tonumber(e_m),
          tonumber(e_d))
