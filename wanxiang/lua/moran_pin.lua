...
local function str_is_chinese(text)
...
function pin_processor.init(env)
  env.pin_enable = env.engine.schema.config:get_bool("moran_pin/enable") or false
...
function pin_processor.func(key_event, env)
  ...
  if cand.type == 'pinned' or str_is_chinese(text) then
...
function pin_filter.init(env)
  ...
  env.indicator = env.engine.schema.config:get_string("moran_pin/indicator") or
      env.engine.schema.config:get_string("super_comment/cand_type/pinned") or "📌"
  user_db.acquire()
end
...
function panacea_translator.init(env)
  ...
  env.infix = env.engine.schema.config:get_string("moran_pin/panacea/infix") or '//'
  env.escaped_infix = string.gsub(env.infix, "([%^%$%(%)%%%.%[%]%*%+%-%?])", "%%%1")
  env.prompt = env.engine.schema.config:get_string("moran_pin/panacea/prompt") or "〔加词〕"
  env.prefix = env.engine.schema.config:get_string("moran_pin/panacea/tip_prefix") or "➕"
  env.indicator = env.engine.schema.config:get_string("moran_pin/indicator") or
      env.engine.schema.config:get_string("super_comment/cand_type/pinned") or "📌"
  env.freestyle = env.engine.schema.config:get_bool("moran_pin/panacea/freestyle") or false
  ...
  local function on_commit(ctx)
    ...
    if str_is_chinese(commit_text) then
...
function panacea_translator.func(input, seg, env)
  ...
  if match then
    local comment = env.prefix .. env.indicator
