        ...
        if file_item then
          if file_item.type == "kList" then
            ...
            for j = 0, list.size - 1 do
              ...
              if p then
                insert(tasks,
                  { path = p, prefix = prefix, conversion = conversion_map, preedit_delim = preedit_delim })
              end
            end
          ...
          for p in s_gmatch(val, split_pat) do
            ...
            if not seen_texts[item_text] then
              ...
              if count <= t.always_qty then
                ...
                insert(always_cands,
                  { cand = abbrev_cand, index = t.always_idx + (count - 1), group_key = group_key, yielded = false })
              else
